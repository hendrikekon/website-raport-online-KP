...
if(isset($_POST['submit'])){
	...
	for ($i=1; $i<=$jumSis; $i++)
	{
	   ...
	   $query = "insert into nilai_uts_uas values('','$id_siswa','$id_guru','$kd_mapel','$id_kelas','$n_uts','$n_uas')";
	   $hasil=mysql_query($query);
	}
	
	if($hasil){
		?><script language="javascript">document.location.href="?page=guru_input_nilai_utsuas_selesai&id_guru=<?php echo $id_guru;?>&id_kelas=<?php echo $id_kelas;?>&kd_mapel=<?php echo $kd_mapel;?>";</script><?php
	}else{
		?><script language="javascript">document.location.href="?page=guru_input_nilai_utsuas_selesai&status=2";</script><?php
	}
...
<tr>
    ...
    <div id="content-table-inner">
      ...
      <br />
        ...
        <?php
		// $view=mysql_query("SELECT * FROM kelas_siswa, siswa, kelas WHERE kelas_siswa.id_siswa=siswa.id_siswa and kelas_siswa.id_kelas=kelas.id_kelas and kelas_siswa.id_kelas='$id_kelas'");

		$view=mysql_query("SELECT * FROM kelas, kelas_siswa, siswa WHERE kelas_siswa.id_kelas=kelas.id_kelas and kelas_siswa.id_siswa=siswa.id_siswa and kelas_siswa.id_kelas='$id_kelas'order by siswa.nama asc");
		...
		while($row=mysql_fetch_array($view)){
			...
			<tr>
				...
				<td><?php echo "<input type='text' name='n_uts".$i."' size='10'/>"; ?></td>
				<td><?php echo "<input type='text' name='n_uas".$i."' size='10'/>"; ?></td>
			</tr>
			<?php
			$i++;
		}
